...
- [X] T030 Run all `quickstart.md` validations V1–V5 locally, then V (Railway) on the live URL
  - Headless V1–V5 *logic* automated + passing: `npm run validate` (15/15), `npm test` (21/21), `npm run smoke` (client loads + full join→combat→respawn path).
  - **Deployed to Railway and verified live**: HTTPS page serves the `v=15` build (200) and a real `wss://` client gets `welcome` + a full 10-plane snapshot at `https://pixel-planes-bryding-production.up.railway.app`. Project owner confirmed it works in-browser.
